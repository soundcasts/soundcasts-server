...
router.get('/soundcast', ({query: {title, userId, regexString}}, res) => {
  if (title && userId && regexString) {
    Soundcast
      .findOne({title, userId, regexString})
      .then(soundcast => soundcast || (new Soundcast(options)).save())
      .then(soundcast => soundcast.needsUpdate() ? soundcast.update() : soundcast)
      .then(soundcast => {
        res.set('Content-Type', 'application/xml').send(soundcast.toXml());
      });
  } else {
    res.status(400).send({
      error: 'you must specify a title (String), userId (Number), and regexString (String)'
    });
  }
});
