...
export default class Soundcast {
  ...
  async getChannelData() {
    ...
    this.channel.image = user.avatar_url.replace(/large/, 't200x200');
  ...
  toXml() {
    var data = {
      ...
      channel: {
        ...
        'itunes:image': {
          '@': {
            href: this.channel.image.replace(/^https/, 'http')
          }
        },
        item: this.channel.tracks.map(track => {
          return {
            ...
            enclosure: {
              '@': {
                //url: `http://soundcasts.net/soundcast?${this.channel.id}/tracks/${track.channel.id}.${track.fileFormat}`,
