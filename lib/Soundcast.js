...
import {formatDuration, makeUrl} from './util';

export default class Soundcast {

  constructor(req) {
    this.req = req;
    var {title, userId, regexString, minDuration} = req.query;

    this.title = title;
    this.userId = userId;
    this.regexString = regexString || '.*';
    this.minDuration = 60000 * (minDuration || 0);
  ...
  async getChannelData() {
    var self = this;
    var sc = new SoundCloud(config.soundcloud.clientId);
    ...
    this.channel.tracks = tracks
      .filter(track => {
        return (
          self.regex.test(track.title)
          && track.streamable
          && track.state === 'finished'
          && track.duration >= self.minDuration
        );
      })
      .map(trackData => {
        var track = {};
        ...
        track.url = `${trackData.stream_url}?client_id=${config.soundcloud.clientId}`;
        track.size = 1;
  ...
  toXml() {
    var data = {
      ...
      channel: {
        ...
        description: `This is a custom podcast featuring the music of ${this.channel.author} created using Soundcasts: https://www.soundcasts.net`,
        'itunes:image': {
          '@': {
