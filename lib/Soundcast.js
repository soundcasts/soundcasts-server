...
schema.methods.toXml = function() {
  var data = {
    ...
    channel: {
      ...
      link: this.link,
      description: `This is a custom podcast featuring the music of ${this.author} created using Soundcasts: https://www.soundcasts.net.\nShort url: https://www.soundcasts.net/${this.id}`,
      'itunes:image': {
        '@': {
          href: this.image
        }
      ...
      item: this.tracks.map((track) => {
        return {
          ...
          enclosure: {
            '@': {
              url: `https://www.soundcasts.net/${this.id}/tracks/${track.id}.${track.fileFormat}`,
              length: track.size,
