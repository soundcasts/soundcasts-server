...
schema.methods.toXml = function() {
  var data = {
    ...
    channel: {
      ...
      link: this.link,
      description: `This is a custom podcast featuring ${this.author} created using Soundcasts: http://soundcasts.com`,
      'itunes:image': {
        '@': {
          href: this.image
        }
      ...
      item: this.tracks.map((track) => {
        return {
          ...
          enclosure: {
            '@': {
              url: `https://bbterivfoi.localtunnel.me/soundcasts/${this.id}/tracks/${track.id}.${track.fileFormat}`,
              length: track.size,
